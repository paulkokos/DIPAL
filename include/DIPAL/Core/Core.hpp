...
public:
  ...
  [[nodiscard]] static constexpr Version getVersion() noexcept {
    return Version(DIPAL_VERSION_MAJOR, DIPAL_VERSION_MINOR,
                   DIPAL_VERSION_PATCH);
  }
  ...
  [[nodiscard]] static std::chrono::high_resolution_clock::time_point
  now() noexcept;
  [[nodiscard]] static double
  elapsedMs(std::chrono::high_resolution_clock::time_point start) noexcept;
  ...
  [[nodiscard]] static constexpr bool isValidDimensions(int width,
                                                        int height) noexcept {
    return isValidDimension(width) && isValidDimension(height);
  }

  [[nodiscard]] static constexpr bool isValidCoordinate(int x, int y, int width,
                                                        int height) noexcept {
  ...
  [[nodiscard]] static bool startsWith(std::string_view str,
                                       std::string_view prefix);
  [[nodiscard]] static bool endsWith(std::string_view str,
  ...
  [[nodiscard]] static void *alignedAlloc(std::size_t size,
                                          std::size_t alignment);
  static void alignedFree(void *ptr);

  template <typename T>
  [[nodiscard]] static std::size_t getAlignedSize(std::size_t count,
...
class ScopedTimer {
public:
  explicit ScopedTimer(std::string_view name = "Timer")
      : m_name(name), m_start(Core::now()) {}
  ...
  [[nodiscard]] double getLastDuration() const noexcept {
