...
TEST_F(BinaryImageTest, CreateBinaryImage) {
    ...
    for (int y = 0; y < 10; ++y) {
        for (int x = 0; x < 10; ++x) {
            ...
            EXPECT_FALSE(pixelResult.value())
...
TEST_F(BinaryImageTest, CloneBinaryImage) {
    ...
    for (int y = 0; y < 10; ++y) {
        for (int x = 0; x < 10; ++x) {
            auto setResult = image.setPixel(x, y, (x + y) % 2 == 0);
            ASSERT_TRUE(setResult) << "Failed to set pixel at (" << x << ", " << y
...
TEST_F(BinaryImageTest, CountWhitePixels) {
    ...
    // Fill all with white
    auto fillResult = image.fill(true);
    ASSERT_TRUE(fillResult) << "Failed to fill image with true: " << fillResult.error().message();
    ...
    // Fill all with black
    auto fillResult2 = image.fill(false);
    ASSERT_TRUE(fillResult2) << "Failed to fill image with false: "
                             << fillResult2.error().message();
    whitePixels = image.countWhitePixels();
...
TEST_F(BinaryImageTest, NonByteAlignedWidth) {
    ...
    auto fillResult = image.fill(true);
    ASSERT_TRUE(fillResult) << "Failed to fill image: " << fillResult.error().message();
    ...
    for (int y = 0; y < 5; ++y) {
        for (int x = 0; x < 10; ++x) {
            auto setResult = image.setPixel(x, y, x % 2 == 0);
            ASSERT_TRUE(setResult) << "Failed to set pixel at (" << x << ", " << y
                                   << "): " << setResult.error().message();
        }
    }
